 ...
 * @version    $Id: Block.php 1052 2017-05-18 09:49:46Z jan.slabon $
 */
...
class SetaPDF_Core_Text_Block
{
    ...
    protected function _drawText(SetaPDF_Core_Canvas $canvas, $x, $y)
    {
        ...
        $text->setWordSpacing($this->_wordSpacing);
        ...
        foreach ($lines AS $lineNo => $line) {
            if ($this->_align !== SetaPDF_Core_Text::ALIGN_LEFT && $line) {
                ...
                if ($this->_wordSpacing != 0) {
                    $stringWidth += ($this->_wordSpacing * substr_count($line, "\x00\x20"));
                }
                ...
                switch ($this->_align) {
                    ...
                    case SetaPDF_Core_Text::ALIGN_JUSTIFY:
                        ...
                        $spaces = substr_count($line, "\x00\x20");
                        if ($spaces > 0) {
                            $text->setWordSpacing(($textWidth - $stringWidth) / $spaces);
                        } else {
                            $text->setWordSpacing(0);
                        }
                }
