 ...
 * @version    $Id: Text.php 1054 2017-05-29 08:10:00Z jan.slabon $
 */
...
class SetaPDF_Core_Canvas_GraphicState_Text
{
    ...
    protected function _showText($text)
    {
        ...
        $chars = $font->getCharsByCharCodes($text, 'UTF-16BE');
        $charCount = count($chars);

        $charCounts = array_count_values($chars);
        // TODO: This needs to be reduced to <20> only and not the unicode value
        $wordCount = isset($charCounts["\x00\x20"]) ? $charCounts["\x00\x20"] : 0;

        $w0 = $font->getGlyphsWidthByCharCodes($text);
