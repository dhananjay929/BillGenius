 ...
 * @version    $Id: Content.php 1052 2017-05-18 09:49:46Z jan.slabon $
 */
...
class SetaPDF_Core_Parser_Content
{
    ...
    public function process()
    {
        ...
        while (($value = $parser->readValue()) !== false) {
            if ($value[0] !== SetaPDF_Core_Parser_RawPdf::TYPE_TOKEN) {
                $this->_stack[] = $value;
                continue;
            }

            if (!isset($this->_operators[$value[1]])) {
                $this->_stack = array();
                continue;
            };

            $result = call_user_func(
                $this->_operators[$value[1]],
    ...
    {
        return $this->getParser()->skipUntilToken($operator);
    }
