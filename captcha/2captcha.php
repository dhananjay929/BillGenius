            ...
            )
{
    ...
    else
    {
	    ...
	    //var_dump($result);
	    if($result == false){
	    	$url_capture = "http://$domain/res.php?key=".$apikey.'&action=get&id='.$captcha_id;
		$curl_init_capture = curl_init();
		curl_setopt($curl_init_capture, CURLOPT_URL, $url_capture);
		curl_setopt($curl_init_capture, CURLOPT_RETURNTRANSFER, true);
		$result = curl_exec($curl_init_capture);
		curl_close($curl_init_capture);
		//echo "<br>Curl Result : ".$result."<br>";
	    }
            if (strpos($result, 'ERROR')!==false)
            {
            	if ($is_verbose) echo "server returned error: $result\n";
